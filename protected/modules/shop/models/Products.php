...
class Products extends CActiveRecord
{
	...
	public function rules()
	{
		return array(
			...
			array('description, specifications,status,end_time,start_time,original,detail,promo_code', 'safe'),
			array('product_id, title, description, price, category_id, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function getImage($image = 0, $thumb = false,$status=null) {
    ...
    public function getTime($start,$end) {
        $time_s=(strtotime($end)-strtotime($start))%60;
        $time_mm=intval((strtotime($end)-strtotime($start))/60);
        $time_m=intval($time_mm%60);
        $time_h=intval($time_mm/60);
        return $time_h.':'.$time_m.':'.$time_s.'-'. date('Y-m-d H:i:s',time());
       // return $date;
    }

    public function getSecond($start,$end) {
        $all_s=strtotime($end)-strtotime(date('Y-m-d H:i:s',time()));
        if($all_s>0)
        return $all_s;
        else
        return '0';
    ...
    public function getDiscount($id) {
        $price=Products::model()->findAllByPk($id);
        $dis=round($price[0]['price']/ $price[0]['original']*100);
        return $dis ;
    }

    public function srt2arr($str) {
