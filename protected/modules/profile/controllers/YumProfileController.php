...
class YumProfileController extends YumController {
	...
	public function accessRules()
	{
		return array(
			...
			array('allow',
				'actions'=>array('view', 'update', 'edit','userpromo'),
				'users' => array('@'),
				),
    ...
    public function actionUserpromo()
    {
        $arrpromo=$this->getpcarr();
        $criteria=new CDbCriteria;
        $criteria->addInCondition('code_id',$arrpromo);
    ...
    public function getpcarr()
    {
        $arrpromo=array();
        $criteria=new CDbCriteria;
        $criteria->addCondition('userId='.Yii::app()->user->id);
        $finpromo=UserCodeNm::model()->findall($criteria);
        foreach($finpromo as $v){
            $arrpromo[]=$v->codeId;
        }
        return $arrpromo;
    }
